...
namespace Filament.WPF6.Helpers
{
    ...
    {
        internal const string regexFindNumberAndUnit = @"(?<number>\d*(\.\d+)?) *(?<units>[a-z\x22]+)";
        ...
        {
            ConvertToWeightEnum convertTo = ConvertToWeightEnum.Grams;
            string defaultUnits;
            if (parameter is string paraString)
            {
                if (Enum.TryParse(paraString, out ConvertToWeightEnum result))
                {
                    convertTo = result;
